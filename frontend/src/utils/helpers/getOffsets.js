...
export const tooltip = (toolBarBounds, selectRangeBounds) => {
  ...
  const getOffsetTop = (place) => {
    ...
    return (
      selectRangeBounds.top +
      window.pageYOffset -
      toolBarBounds.height +
      toolBarBounds.height +
      selectRangeBounds.height +
      margin
    );
  };
  ...
  const getOffsetLeft = (place) => {
    ...
    if (place.center) {
      return (
        selectRangeBounds.left -
        (toolBarBounds.width - selectRangeBounds.width) / 2
      );
    }
    if (place.left) {
      return (
  ...
  const checkPlaceHorizontal = () => {
    ...
    if (
      selectRangeBounds.left >
        (toolBarBounds.width - selectRangeBounds.width) / 2 &&
      windowWidth - selectRangeBounds.right > toolbarWidth
    ) {
      return {
        right: true,
        center: true,
