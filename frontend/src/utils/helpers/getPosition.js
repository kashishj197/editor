...
export default (rootNode, popupNode, customOptions) => {
  ...
  const options = {
    direction:
      customOptions && customOptions.direction
    ...
    position:
      customOptions && customOptions.position ? customOptions.position : false,
  };
  ...
  if (root && !_.isNull(root) && popup && !_.isNull(popup)) {
    ...
    if (options.direction === "left") {
      ...
      if (options.position) {
        ...
        left =
          rootElementBounding.left -
          (popupElementBounding.width +
            popupElementBounding.width * options.scale) -
          options.margin;
      }
    }
    ...
    if (options.direction === "center bottom") {
      ...
      left =
        rootElementBounding.left +
        rootElementBounding.width / 2 -
        popupElementBounding.width / 2;
    }
    ...
    if (options.direction === "right bottom") {
      top =
        ...
        options.margin;
      left =
        rootElementBounding.right - popupElementBounding.width - options.margin;
    }
