...
import "../../../styles/elements/_rangeslider_tooltip.scss";
import Slider from "@appigram/react-rangeslider";
import classNames from "classnames";
import PropTypes from "prop-types";
import _ from "lodash";

const RangeSliderTooltip = ({ min, max, value, step, input, onChange }) => {
  ...
  useEffect(
    () => () => {
      if (
        inputNode &&
        inputNode.current &&
        Number(inputNode.current.value) !== Number(inputValue)
      ) {
  ...
  const onlyDigits = (string) =>
    string ? string.substr(0, String(max).length).replace(/[^\d]/g, "") : "";

  // eslint-disable-next-line
  const onlyDigitsWithDots = (string) =>
    string
      ? string.substr(0, String(max).length + 1).replace(/[^\d\.]/g, "")
      : "";
